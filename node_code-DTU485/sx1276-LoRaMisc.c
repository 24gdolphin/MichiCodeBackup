...
uint8_t SX1276LoRaGetSignalBandwidth( void ) //带宽
{
    SX1276Read( REG_LR_MODEMCONFIG1, &SX1276LR->RegModemConfig1 );
...
uint8_t SX1276LoRaGetSpreadingFactor( void ) //扩频因子
{
    SX1276Read( REG_LR_MODEMCONFIG2, &SX1276LR->RegModemConfig2 );   
...
void SX1276LoRaSetErrorCoding( uint8_t value ) //纠错码
{
    SX1276Read( REG_LR_MODEMCONFIG1, &SX1276LR->RegModemConfig1 );
...
void SX1276LoRaSetPacketCrcOn( bool enable ) //CRC
{
    SX1276Read( REG_LR_MODEMCONFIG2, &SX1276LR->RegModemConfig2 );
...
void SX1276LoRaSetPreambleLength( uint16_t value ) //前导码长度
{
...
void SX1276LoRaSetImplicitHeaderOn( bool enable )  //显式/隐式报头
{
    SX1276Read( REG_LR_MODEMCONFIG1, &SX1276LR->RegModemConfig1 );
...
bool SX1276LoRaGetImplicitHeaderOn( void )  //显式/隐式报头
{
    SX1276Read( REG_LR_MODEMCONFIG1, &SX1276LR->RegModemConfig1 );
...
void SX1276LoRaSetHopPeriod( uint8_t value )  //跳频周期
{
...
void SX1276LoRaSetPayloadLength( uint8_t value ) //负载长度最大值
{
...
void SX1276LoRaSetSymbTimeout( uint16_t value )  //设置RX超时有效位  （最高，最低在两个寄存器里）
{
